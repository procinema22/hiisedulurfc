  ...
  */
  ...
  /* ---------------------------
    ...
    function safeParseInt(val, def = 0) {
  ...
  drawH = ih * scaleBase * scale;
    ...
    /* ---------------------------
        ...
        --------------------------- */
        let marginTopPx = (parseFloat(marginTop?.value || 5) / 10) * PX_PER_CM;
        let marginBottomPx = (parseFloat(marginBottom?.value || 5) / 10) * PX_PER_CM;
        let marginLeftPx = (parseFloat(marginLeft?.value || 5) / 10) * PX_PER_CM;
        let marginRightPx = (parseFloat(marginRight?.value || 5) / 10) * PX_PER_CM;
    /* ---------------------------
    ...
    /* ---------------------------
        ...
        async function autoPreview() {
          ...
          // hanya render ulang kalau belum ada cache
          if (!lastRenderCache) {
            lastRenderCache = await renderAllPagesToCanvases();
          }
        
          pagesCache = lastRenderCache.pages || [];
          showPageAtIndex(0);
          await updatePricePreview();
        }
    ...
    if (manualHargaCheckbox) {
      ...
      manualHargaCheckbox.disabled = false;
      if (manualHargaBox) manualHargaBox.style.display = 'block';
