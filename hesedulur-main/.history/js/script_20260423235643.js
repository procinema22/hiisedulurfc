  ...
  */
  ...
  drawH = ih * scaleBase * scale;
    ...
    /* ---------------------------
        ...
        --------------------------- */
        let marginTopPx, marginBottomPx, marginLeftPx, marginRightPx;

        if (advancedMargin && advancedMargin.checked) {
          marginTopPx = (parseFloat(marginTop.value || 5) / 10) * pxPerCm;
          marginBottomPx = (parseFloat(marginBottom.value || 5) / 10) * pxPerCm;
          marginLeftPx = (parseFloat(marginLeft.value || 5) / 10) * pxPerCm;
          marginRightPx = (parseFloat(marginRight.value || 5) / 10) * pxPerCm;
        } else {
          const m = (Math.max(0, parseFloat(marginInputMm ? marginInputMm.value : 5) || 5) / 10) * pxPerCm;
          marginTopPx = marginBottomPx = marginLeftPx = marginRightPx = m;
        }
