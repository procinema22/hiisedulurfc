...
async function updatePricePreview() {
  try {
    ...
    console.error("Harga gagal:", err);
  }
}
// ===============================
// COPY SYSTEM FIX FINAL
// Auto detect input/select copy
// ===============================

// ambil elemen copy dari beberapa kemungkinan id
const copyInput =
  document.getElementById("copyInput") ||
  document.getElementById("copy") ||
  document.getElementById("copyCount") ||
  document.getElementById("jumlahCopy");

const copySelect =
  document.getElementById("copySelect") ||
  document.getElementById("copyMenu");
...
async function addFilesToBatch(files) {
  ...
  const mode =
    (typeof modeSelect !== "undefined" && modeSelect)
      ? modeSelect.value
      : "normal";

  // ambil nilai copy aman
  let copyValue = 1;

  if (copyInput && copyInput.value) {
    copyValue = parseInt(copyInput.value) || 1;
  } else if (copySelect && copySelect.value) {
    copyValue = parseInt(copySelect.value) || 1;
  }

  // minimal 1
  if (copyValue < 1) copyValue = 1;
  ...
  // lanjut proses kamu di bawah sini...
}
...
function showPageAtIndex(i) {
  ...
  currentPageIndex = Math.max(
    0,
    Math.min(i, pagesCache.length - 1)
  );
  ...
  if (nextPageBtn) {
    nextPageBtn.disabled =
      currentPageIndex === pagesCache.length - 1;
  }
}
...
if (nextPageBtn) {
  nextPageBtn.onclick = () =>
    ...
    if (resetBtn) resetBtn.addEventListener("click", async (e) => {
      ...
      if (marginTop) marginTop.value = "5";
      if (marginBottom) marginBottom.value = "5";
      if (marginLeft) marginLeft.value = "5";
      if (marginRight) marginRight.value = "5";
    ...
    /* ---------------------------
      ...
      try {
        ...
        if (!hideInfo.checked) {
          ...
          // =========================
          // FOOTER FINAL + SAFE AREA
          // =========================
        
          const footerHeight = 120;          // tinggi footer
          const footerY = fullH - footerHeight;
          const footerTopLimit = footerY - 40; // batas aman foto
        
          // garis atas footer
          lastCtx.beginPath();
          lastCtx.moveTo(0, footerTopLimit);
          lastCtx.lineTo(fullW, footerTopLimit);
          lastCtx.strokeStyle = "#111";
          lastCtx.lineWidth = 3;
          lastCtx.stroke();
          ...
          // JUDUL KIRI
          // =========================
          lastCtx.font = "bold 34px Poppins";
          lastCtx.fillStyle = "#111";
          lastCtx.textAlign = "left";
        
          lastCtx.fillText(
            "SEDULUR FOTO COPY & PRINTING",
            40,
            footerY
          );
          ...
          // LABEL KECIL
          // =========================
          lastCtx.font = "18px Poppins";
          lastCtx.fillStyle = "#666";
        
          lastCtx.fillText("NAMA PEMESAN", 40, footerY + 28);
          lastCtx.fillText("HARGA", fullW - 170, footerY + 28);
        
          // =========================
          // ISI DATA
          // =========================
          lastCtx.font = "bold 28px Poppins";
          lastCtx.fillStyle = "#111";
        
          // nama kiri
          lastCtx.fillText(
            userName.value?.trim() || "Pemesan",
            40,
            footerY + 58
          );
    ...
    [marginTop, marginBottom, marginLeft, marginRight].forEach(el => {
      if (el) el.oninput = autoPreview;
