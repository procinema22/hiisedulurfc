...
for (const batch of batches) {
  ...
  if (mode === "circle") {
    ...
    // simpan diameter per batch, bukan global input
    ...
    for (let cp = 0; cp < Math.max(1, batch.copy || 1); cp++) {
      for (const file of batch.files) {
        ...
        // pindah baris jika melebihi kanan
        ...
        // pindah halaman jika melebihi bawah
    ...
    if (mode === 'circle') {
      ...
      for (let cp = 0; cp < Math.max(1, batch.copy || 1); cp++) {
        for (const file of batch.files) {
          ...
          if (x + diameterPx > fullW - mr) { x = ml; y += rowMaxH + gap; rowMaxH = 0; }
          if (y + diameterPx > fullH - mb) { pageIdx++; placementsByPage[pageIdx] = [];y = marginPx;; y = marginPx; rowMaxH = 0; }
          const key = fileKeyFor(file);
          const saved = loadSavedForKey(key);
          placementsByPage[pageIdx].push({
      ...
      for (let cp = 0; cp < Math.max(1, batch.copy || 1); cp++) {
        for (const file of batch.files) {
          const imgObj = await loadImageWithEXIF(file, 'preview'); if (!imgObj) continue;
          if (x + boxW > fullW - mr) { x = marginPx; y += rowMaxH + gap; rowMaxH = 0; }
          if (y + boxH > fullH - mb) { pageIdx++; placementsByPage[pageIdx] = []; x = marginPx; y = marginPx; rowMaxH = 0; }
          const key = fileKeyFor(file);
          const saved = loadSavedForKey(key);
          placementsByPage[pageIdx].push({
    ...
    /* ---------------------------
      ...
      try {
        ...
        if (!hideInfo.checked) {
          ...
          const footerY = fullH - 120;
        
          // garis atas footer
          lastCtx.beginPath();
          lastCtx.moveTo(0, footerY - 40);
          lastCtx.lineTo(fullW, footerY - 40);
          ...
          lastCtx.fillText("SEDULUR FOTO COPY & PRINTING", 40, footerY);
          ...
          lastCtx.fillText("TANGGAL", fullW / 2 - 80, footerY + 28);
          ...
          lastCtx.fillText(userName.value || "Pemesan", 40, footerY + 58);
          lastCtx.fillText("Rp " + totalHarga.toLocaleString(), fullW - 170, footerY + 58);
